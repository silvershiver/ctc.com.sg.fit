			...
			<tr id="room_selectionID">
				...
				<td colspan="2" style="background:#eff0f1; text-align:center; font-size:14px; border-right:1px solid black">
					<div>
						...
						?>
							<select name="adultQty<?php echo $x; ?>" id="adultQty" style="width:50px">
...
?>
		...
		?>
			...
			<div style="background-color:#FFBABA; padding:10px; font-size:14px; color:red">
				Room <?php echo $r; ?> maximum capacity is <?php echo $roomCombinationQty; ?> pax(s). Please update your pax statement for this room.
			</div>
			<br />
			<!--End of Notification-->
		<?php
			...
			<table border='0' cellpadding='0' cellspacing='0' style="width:100%">
				...
				?>
					<tr class='days afterPrice'>
						<td style="text-align:center; background:#eff0f1;">
							<div style="font-size:12px">
								<?php echo $paxGetQty[0]; ?> x Adult(s) Price
							</div>
						</td>
				...
				?>
					<tr class='days afterPrice'>
						<td style="text-align:center; background:#eff0f1;">
							<div style="font-size:12px">
								<?php echo $paxGetQty[1]; ?> x Child(s) (With Bed) Price
							</div>
						</td>
						<td style="text-align:right; background:#eff0f1;">
							<div style="font-size:12px">
								$<?php echo number_format($paxGetQty[1]*$child_wb_price, 2); ?>
							</div>
						</td>
					</tr>
				<?php
				...
				?>
					<tr class='days afterPrice'>
						<td style="text-align:center; background:#eff0f1;">
							<div style="font-size:12px">
								<?php echo $paxGetQty[2]; ?> x Child(s) (Without Bed) Price
							</div>
						</td>
						<td style="text-align:right; background:#eff0f1;">
							<div style="font-size:12px">
				...
				?>
					<tr class='days afterPrice'>
						<td style="text-align:center; background:#eff0f1;">
							<div style="font-size:12px"><?php echo $paxGetQty[3]; ?> x Infant(s) Price</div>
						</td>
						<td style="text-align:right; background:#eff0f1;">
							<div style="font-size:12px">
				...
				<tr class='days afterPrice'>
					...
					<td style="text-align:right; background:#eff0f1;">
						<?php
						$totalPrice = ($paxGetQty[0]*$adultPrice)+($paxGetQty[1]*$child_wb_price)+($paxGetQty[2]*$child_wob_price)+($paxGetQty[3]*$infant_price);
						$totalGrandPrice += ($paxGetQty[0]*$adultPrice)+($paxGetQty[1]*$child_wb_price)+($paxGetQty[2]*$child_wob_price)+($paxGetQty[3]*$infant_price);
						?>
						<div style="font-size:12px">

...
<div style="width: 880px; margin:0 auto">
	...
	$connection = mysqli_connect(DB_LOCALHOST, DB_USERNAME, DB_PASSWORD, DB_NAME);
	$payment_res = mysqli_query(
		$connection, "SELECT * FROM payment_reference WHERE bookOrderID = '".$bookingOrder."'"
	);
	if( mysqli_num_rows($payment_res) > 0 ) {
		while( $payment_row = mysqli_fetch_array($payment_res, MYSQL_ASSOC) ) {
			$p_created 	  	  = $payment_row["created"];
			$p_TM_PaymentType = $payment_row["TM_PaymentType"];
		}
	}
	/*
